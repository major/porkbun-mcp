...
def _register_dns_readonly() -> tuple[FastMCP, PorkbunMCPSettings]:
    mcp = FastMCP("test")
...
def _register_dns_write() -> tuple[FastMCP, PorkbunMCPSettings]:
    mcp = FastMCP("test")
...
class TestDNSCreate:
    ...
    async def test_dns_create_blocked_without_get_muddy(self, mock_context: MagicMock) -> None:
    ...
    async def test_dns_create_invalid_type(self, mock_context_write: MagicMock) -> None:
...
class TestDNSDelete:
    ...
    async def test_dns_delete_blocked_without_get_muddy(self, mock_context: MagicMock) -> None:
        """dns_delete should raise ToolError without --get-muddy."""
        mcp, _ = _register_dns_readonly()
        tool_fn = get_tool_fn(mcp, "dns_delete")

        with pytest.raises(ToolError, match="--get-muddy"):
            await tool_fn(mock_context, domain="example.com", record_id="12345")
    ...
    async def test_dns_delete_success(
        ...
        result = await tool_fn(mock_context_write, domain="example.com", record_id="12345")
